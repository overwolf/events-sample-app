...
import { GameClosedEvent, GameLaunchedEvent, PostGameEvent, RunningGame } from '../interfaces/running-game';
...
export class GameDetectionService extends EventEmitter {
  ...
  public init() {
    ...
    overwolf.games.onGameInfoUpdated.addListener((update) => this.gameUpdated(update));
    // Get the currently running game (if any)
    overwolf.games.getRunningGameInfo2((info) => {
      // If there is a running game, run the game launched logic for a non-fresh launch
   ...
   * 
   * @param {overwolf.games.RunningGameInfo} gameInfo - The GameInfo of the new game
   * @param {boolean} freshLaunch - Is this a "fresh launch", or was the game already open before it was detected? (For example, the app opened after the game)
   */
  private gameLaunched(gameInfo: overwolf.games.RunningGameInfo, freshLaunch: boolean) {
    // Ensure that fresh launch was not called while there was a running game beforehand
    if (freshLaunch && this._runningGame) throw new Error(`A fresh launch was called, but a running game was already detected! 
    Launched \`${gameInfo.title}\`, while \`${this._runningGame.name}\` was already running`);
   ...
   * 
   * *Alternatively - should we run post-game logic, or did the active game simply change*
   */
  private gameClosed(fullShutdown: boolean) {
    // Ensure that there is a game running before running `gameClosed` logic
    if (!this._runningGame) throw new Error(`Cannot run \`gameClosed\` when no game is currently running!`);
   ...
   * 
   * @param {overwolf.games.GameInfoUpdatedEvent} updateEvent - The GameInfo updated event
   */
  private gameUpdated(updateEvent: overwolf.games.GameInfoUpdatedEvent) {
    /**
     * Did a new game just get launched?
     * 
     * This could technically be done using `overwolf.games.onGameLaunched`. However, as we already need to utilize `overwolf.games.onGameInfoUpdated`
     * to detect if a game was terminated, it is easier to just use it for both.
     */
    if(updateEvent.reason.includes(overwolf.games.enums.GameInfoChangeReason.GameLaunched)) {
      // Is there a game already running?
      if(this._runningGame){
        // Run game closed cleanup, without running post-game logic
        this.gameClosed(false);
      }
      // Run game launched logic for the new launched game, as a fresh launch (as it was detected from the start)
      this.gameLaunched(updateEvent.gameInfo as overwolf.games.RunningGameInfo, true);
    }
    // If the game was terminated
    else if (updateEvent.reason.includes(overwolf.games.enums.GameInfoChangeReason.GameTerminated)) {
